...
function buildResponseBlocks(results, intent, aiSummary, aiSummaryDegraded) {
  const blocks = [];

  // Header
  blocks.push(headerBlock(results));

  // Row 18 degraded marker: the summary step failed because the LLM was
  // unavailable — the deterministic findings below are complete; only the
  // narrative layer is missing. Labeled so degradation is never silent.
  if (aiSummaryDegraded && !aiSummary) {
    blocks.push({
      type: "context",
      elements: [
        {
          type: "mrkdwn",
          text: ":warning: *AI summary unavailable — LLM unreachable.* Deterministic findings only; retry for the narrative summary.",
        },
      ],
    });
  }
